...
namespace Beskar.Memory.Serialization.Serializers.Collections.Concurrent;

/// <summary>
/// Serializer for ConcurrentQueue values.
/// </summary>
public abstract class ConcurrentQueueSerializer<T> : ISerializer<ConcurrentQueue<T>?>
{
   public static int Write(ref BufferWriter<byte> writer, scoped in ConcurrentQueue<T>? value)
   {
      if (value is null)
      {
   ...
   public static bool TryRead(ref SequenceReader<byte> reader, out ConcurrentQueue<T>? value)
   {
   ...
   public static int CalculateByteLength(scoped in ConcurrentQueue<T>? value)
   {
      if (value is null)
      {
         return sizeof(int);
