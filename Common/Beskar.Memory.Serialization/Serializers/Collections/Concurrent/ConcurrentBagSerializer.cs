...
/// <summary>
/// Serializer for ConcurrentBag values.
/// </summary>
public abstract class ConcurrentBagSerializer<T> : ISerializer<ConcurrentBag<T>?>
{
   public static int Write(ref BufferWriter<byte> writer, scoped in ConcurrentBag<T>? value)
   {
      if (value is null)
      {
   ...
   public static bool TryRead(ref SequenceReader<byte> reader, out ConcurrentBag<T>? value)
   {
   ...
   public static int CalculateByteLength(scoped in ConcurrentBag<T>? value)
   {
      if (value is null)
      {
         return sizeof(int);
