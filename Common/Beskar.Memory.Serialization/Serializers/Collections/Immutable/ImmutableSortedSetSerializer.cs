...
/// <summary>
/// Serializer for ImmutableSortedSet values.
/// </summary>
public abstract class ImmutableSortedSetSerializer<T> : ISerializer<ImmutableSortedSet<T>?>
{
   public static int Write(ref BufferWriter<byte> writer, scoped in ImmutableSortedSet<T>? value)
   {
      if (value is null)
      {
   ...
   public static bool TryRead(ref SequenceReader<byte> reader, out ImmutableSortedSet<T>? value)
   {
   ...
   public static int CalculateByteLength(scoped in ImmutableSortedSet<T>? value)
   {
      if (value is null)
      {
         return sizeof(int);
