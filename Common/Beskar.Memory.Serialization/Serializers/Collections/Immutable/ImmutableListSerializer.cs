...
/// <summary>
/// Serializer for ImmutableList values.
/// </summary>
public abstract class ImmutableListSerializer<T> : ISerializer<ImmutableList<T>?>
{
   public static int Write(ref BufferWriter<byte> writer, scoped in ImmutableList<T>? value)
   {
      if (value is null)
      {
   ...
   public static bool TryRead(ref SequenceReader<byte> reader, out ImmutableList<T>? value)
   {
   ...
   public static int CalculateByteLength(scoped in ImmutableList<T>? value)
   {
      if (value is null)
      {
         return sizeof(int);
