...
/// <summary>
/// Serializer for ReadOnlyDictionary values.
/// </summary>
public abstract class ReadOnlyDictionarySerializer<TKey, TValue> : ISerializer<ReadOnlyDictionary<TKey, TValue>?>
   where TKey : notnull
{
   public static int Write(ref BufferWriter<byte> writer, scoped in ReadOnlyDictionary<TKey, TValue>? value)
   {
      if (value is null)
      {
   ...
   public static bool TryRead(ref SequenceReader<byte> reader, out ReadOnlyDictionary<TKey, TValue>? value)
   {
      ...
      value = new ReadOnlyDictionary<TKey, TValue>(dictionary);
      return true;
   }

   public static int CalculateByteLength(scoped in ReadOnlyDictionary<TKey, TValue>? value)
   {
      if (value is null)
      {
         return sizeof(int);
