...
/// </summary>
public abstract class DictionarySerializer<TKey, TValue> : ISerializer<Dictionary<TKey, TValue>?>
   where TKey : notnull
{
   public static int Write(ref BufferWriter<byte> writer, scoped in Dictionary<TKey, TValue>? value)
   {
      if (value is null)
      {
   ...
   public static bool TryRead(ref SequenceReader<byte> reader, out Dictionary<TKey, TValue>? value)
   {
   ...
   public static int CalculateByteLength(scoped in Dictionary<TKey, TValue>? value)
   {
      if (value is null)
      {
         return sizeof(int);
