...
/// <summary>
/// Serializer for LinkedList values.
/// </summary>
public abstract class LinkedListSerializer<T> : ISerializer<LinkedList<T>?>
{
   public static int Write(ref BufferWriter<byte> writer, scoped in LinkedList<T>? value)
   {
      if (value is null)
      {
   ...
   public static bool TryRead(ref SequenceReader<byte> reader, out LinkedList<T>? value)
   {
   ...
   public static int CalculateByteLength(scoped in LinkedList<T>? value)
   {
      if (value is null)
      {
         return sizeof(int);
