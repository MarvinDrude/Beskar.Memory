...
/// <summary>
/// Serializer for SortedList values.
/// </summary>
public abstract class SortedListSerializer<TKey, TValue> : ISerializer<SortedList<TKey, TValue>?>
   where TKey : notnull
{
   public static int Write(ref BufferWriter<byte> writer, scoped in SortedList<TKey, TValue>? value)
   {
      if (value is null)
      {
   ...
   public static bool TryRead(ref SequenceReader<byte> reader, out SortedList<TKey, TValue>? value)
   {
      ...
      var sortedList = new SortedList<TKey, TValue>(length);
   ...
   public static int CalculateByteLength(scoped in SortedList<TKey, TValue>? value)
   {
      if (value is null)
      {
         return sizeof(int);
